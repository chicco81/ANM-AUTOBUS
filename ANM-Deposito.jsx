...
// Una ripresa ha: linea, oraUscita, oraRientro, tipoRientro, mezzi:[{id,mezzo,motivo}]
...
// tutti i mezzi usati in una giornata (da tutti i trip/mezzi)
function getAllMezziFromVehicles(vehicles) {
  const all = [];
  vehicles.forEach(v => v.trips?.forEach(tr => tr.mezzi?.forEach(m => {
    if (m.mezzo) all.push(m.mezzo);
  })));
  return [...new Set(all)];
}
...
// ── Sotto-componente: lista mezzi di una ripresa (editabile) ─────────────────
        ...
        return (
          <div key={m.id} style={{
            ...
            <div style={{flex:1}}>
              ...
              {/* motivo cambio — solo dal 2° mezzo in poi */}
              {idx>0 && (
            ...
            {/* rimuovi mezzo — solo se >1 e non readOnly */}
...
export default function App() {
  ...
  function saveNewVehicle() {
    ...
    const newV = {
      ...
      // "mezzo" principale = primo mezzo della prima ripresa (per compatibilità UI)
  ...
  const filtered = vehicles.filter(v=>{
    // cerca in tutti i mezzi di tutte le riprese
  ...
  function addFleetMezzo(mezzo,tipo,stato,note) {
    ...
    setFleet(p=>[{id:genId(),mezzo:mezzo.trim().toUpperCase(),tipo,stato,note,
      aggiornato:getToday()},...p]);
  }
  ...
  return (
    <div style={S.root}>
      {/* HEADER — solo logo + tema, niente bottoni azione */}
      ...
      {section==="dashboard"&&view==="list"&&(
        <div style={{
          background:t.headerBg,borderBottom:`1px solid ${t.navBorder}`,
            ...
            <div style={S.content}>
              ...
              {filtered.length===0?(
                <div style={{textAlign:"center",color:t.textMuted,padding:"50px 0"}}>
                  ...
                  <div style={{fontSize:12,marginTop:6,color:t.textSub}}>
                    Premi "+ Nuova scheda" per iniziare.
                  </div>
                </div>
              ):(
                      ...
                      <div key={v.id} style={S.vehicleCard}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",
                          ...
                          <div>
                            ...
                            {/* mostra tutti i mezzi del primo trip */}
      ...
      {section==="history"&&(
        <HistoryView t={t} S={S} history={history} fleet={fleet} onDelete={deleteHistoryDay}/>
      )}
    </div>
  );
}
        ...
        {formTrips.map((tr,idx)=>(
          <div key={tr.id} style={{...S.tripFormBox,borderLeftColor:t.accentBlue,marginBottom:14}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
              <span style={{fontSize:13,fontWeight:700,color:t.accentBlue,letterSpacing:1}}>
                RIPRESA {idx+1}
              </span>
              {formTrips.length>1&&(
  ...
  return (
    <div style={{maxWidth:960,margin:"0 auto",padding:"20px 16px"}}>
      <div style={S.formCard}>
        ...
        {v.trips.map((tr,idx)=>{
          ...
          return (
            <div key={tr.id} style={{...S.tripCardDetail,borderLeftColor:t.accentBlue,marginBottom:14}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  <span style={{fontSize:11,color:t.textSub,letterSpacing:1,textTransform:"uppercase"}}>
                    Ripresa {idx+1}
                  </span>
                ...
                <button onClick={()=>{if(confirm(`Eliminare ripresa ${idx+1}?`))onDeleteTrip(v.id,tr.id);}}
                  style={{background:"transparent",border:"none",color:"#ef4444",
                    cursor:"pointer",fontSize:14,fontFamily:"inherit"}}>🗑️</button>
              </div>

              {/* mezzi della ripresa — READ ONLY con aggiornamento inline */}
              <div style={{marginBottom:12}}>
                <div style={{fontSize:10,color:t.textMuted,letterSpacing:1,textTransform:"uppercase",marginBottom:8}}>
                  Mezzi utilizzati
                </div>
                <MezziRipresa
              ...
              <div style={{display:"flex",alignItems:"flex-start",gap:14,flexWrap:"wrap"}}>
                ...
                <div style={{flex:1}}>
                  ...
                  {hasR?(
                    <div>
                      <div style={{fontSize:22,fontWeight:700,color:tOpt?.color||"#22c55e",letterSpacing:2}}>
                        {tr.oraRientro}
                      </div>
        ...
        {showAdd&&(
          <div style={{background:t.tripCardBg,border:`1px dashed ${t.cardBorder}`,
                       borderRadius:8,padding:14,marginTop:10}}>
            <div style={{fontSize:12,fontWeight:700,color:t.accentBlue,letterSpacing:1,marginBottom:12}}>
              NUOVA RIPRESA
            </div>
            <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:12}}>
              <div style={{flex:2,minWidth:100}}>
            ...
            <div style={{fontSize:11,color:t.textMuted,letterSpacing:1,textTransform:"uppercase",marginBottom:8}}>
              Mezzi di questa ripresa
            </div>
            <MezziRipresa
              mezzi={newTrip.mezzi}
              onChange={m=>setNewTrip(p=>({...p,mezzi:m}))}
              fleet={fleet} t={t} S={S}/>
            <button onClick={handleAddTrip} style={{...S.btnPrimary,marginTop:12}}>
              ✅ Aggiungi ripresa
            </button>
          </div>
        )}
        ...
        {v.note&&(
          <div style={{background:t.tripCardBg,border:`1px solid ${t.cardBorder}`,
...
function FleetView({t,S,fleet,onAdd,onUpdateStato,onDelete,history}) {
  ...
  const cnt={
    totale:fleet.length,
    buono:fleet.filter(f=>f.stato==="buono").length,
      ...
      <div style={{...S.formCard,marginBottom:18}}>
        ...
        <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:10}}>
          <div style={{flex:2,minWidth:100}}>
            ...
            <input style={S.input} placeholder="es. BC123"
              value={newMezzo} onChange={e=>setNewMezzo(e.target.value)}/>
          </div>
          <div style={{flex:1,minWidth:80}}>
        ...
        <input style={{...S.input,marginBottom:10}} placeholder="Note (opzionale)"
          value={newNote} onChange={e=>setNewNote(e.target.value)}/>
        <button onClick={()=>{onAdd(newMezzo,newTipo,newStato,newNote);setNewMezzo("");setNewNote("");}}
          style={S.btnPrimary}>✅ Aggiungi</button>
      </div>
      ...
      <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:12,flexWrap:"wrap"}}>
        ...
        <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
          {[["tutti","Tutti"],["buono","✅ Buoni"],["guasto","⚠️ Guasti"],
            ...
            <button key={k} onClick={()=>setFilterStato(k)}
              style={{...S.tab,...(filterStato===k?S.tabActive:{})}}>
              {l}
            </button>
          ))}
        </div>
          ...
          {filtered.map(f=>{
            ...
            return (
              <div key={f.id} style={{...S.vehicleCard,borderLeft:`3px solid ${so.color}`}}>
                ...
                {f.faultNote&&!isEditing&&(
                  <div style={{fontSize:12,color:"#ef4444",marginBottom:6}}>⚠️ {f.faultNote}</div>
                )}
                {f.note&&!isEditing&&(
                  <div style={{fontSize:12,color:t.textMuted,marginBottom:6,fontStyle:"italic"}}>{f.note}</div>
                )}
                {isEditing?(
                  <div style={{borderTop:`1px solid ${t.cardBorder}`,paddingTop:10,marginTop:6}}>
                    ...
                    <input style={{...S.input,marginBottom:10}} placeholder="Note guasto..."
                      value={editNote} onChange={e=>setEditNote(e.target.value)}/>
                    <div style={{display:"flex",gap:8}}>
                      <button onClick={()=>{onUpdateStato(f.id,editStato,editNote);setEditId(null);}}
                        style={S.btnSmall}>✅ Salva</button>
                      <button onClick={()=>setEditId(null)}
                        style={{...S.btnSecondary,padding:"5px 12px",fontSize:12}}>Annulla</button>
                    </div>
                  </div>
                ):(
...
// HISTORY VIEW
// ════════════════════════════════════════════════════════════════════════════
function HistoryView({t,S,history,fleet,onDelete}) {
  const [search,setSearch]         = useState("");
        ...
        (day.mezzi||[]).flatMap(v=>
          (v.trips||[]).flatMap(tr=>
            (tr.mezzi||[]).filter(m=>m.mezzo.toLowerCase().includes(searchMezzo.toLowerCase()))
              .map(m=>({...v,_mezzo:m.mezzo,_data:day.data,_dayId:day.id,
  ...
  return (
    <div style={S.content}>
        ...
        {searchMezzo.trim().length>1&&(
          mezzoResults.length===0?(
            <div style={{color:t.textMuted,fontSize:13,marginTop:10}}>
              Nessun utilizzo trovato per "{searchMezzo}".
            </div>
          ):(
            <div style={{marginTop:12}}>
              <div style={{fontSize:12,color:t.textMuted,marginBottom:8}}>
                {mezzoResults.length} scheda/e trovata/e:
              </div>
              {mezzoResults.map((v,i)=>(
                <div key={i} style={{background:t.tripCardBg,border:`1px solid ${t.cardBorder}`,
                                     borderRadius:8,padding:12,marginBottom:8}}>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                    <span style={{fontWeight:700,color:t.accent}}>📅 {v._data}</span>
                    {v.fault&&v.fault!=="Nessun guasto"&&
                      <span style={{fontSize:11,color:"#ef4444"}}>⚠️ {v.fault}</span>}
                  </div>
                  {v._trips.map((tr,j)=>{
                    const tOpt=tr.tipoRientro?TIPO_RIENTRO[tr.tipoRientro]:null;
                    ...
                    return (
                      <div key={j} style={{fontSize:12,color:tr._highlight?t.text:t.textMeta,
                                           ...
                                           fontWeight:tr._highlight?600:400}}>
                        <span style={{background:`${t.accentBlue}22`,borderRadius:4,padding:"0 6px",
                          fontSize:11,color:t.accentBlue,fontWeight:700}}>{j+1}</span>
      ...
      <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:12}}>
        <input style={S.searchInput} placeholder="🔍 Filtra per data o mezzo..."
          value={search} onChange={e=>setSearch(e.target.value)}/>
      ...
      {filteredDays.length===0?(
        <div style={{textAlign:"center",color:t.textMuted,padding:"40px 0"}}>
          ...
          <div style={{fontSize:12,marginTop:6,color:t.textSub}}>
            Usa "📦 Archivia" nella schermata Giornata per salvare.
          </div>
        </div>
            ...
            {isOpen&&(
              <div style={{borderTop:`1px solid ${t.cardBorder}`,marginTop:12,paddingTop:12}}>
                {(day.mezzi||[]).map(v=>{
                  ...
                  const sc=STATUS_COLORS[status]||"#94a3b8";
                  return (
                    <div key={v.id} style={{background:t.tripCardBg,border:`1px solid ${t.cardBorder}`,
                                            borderRadius:8,padding:12,marginBottom:8}}>
                      <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                        <Badge color={sc} label={status} small/>
                        {v.fault&&v.fault!=="Nessun guasto"&&
                      ...
                      {v.trips?.map((tr,j)=>{
                        ...
                        return (
                          <div key={j} style={{marginBottom:8,paddingBottom:8,
                            borderBottom:j<v.trips.length-1?`1px solid ${t.cardBorder}`:"none"}}>
                            <div style={{display:"flex",gap:7,flexWrap:"wrap",alignItems:"center",marginBottom:4}}>
                              <span style={{background:`${t.accentBlue}22`,borderRadius:4,padding:"0 6px",
                                fontSize:11,color:t.accentBlue,fontWeight:700}}>R{j+1}</span>
                            ...
                            <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap",marginLeft:24}}>
                              {mList.map((mz,mi)=>(
                                <span key={mi} style={{display:"flex",alignItems:"center",gap:5}}>
                                  ...
                                  <span style={{fontWeight:700,color:mi===0?t.accent:"#f59e0b",
                                    fontSize:13,letterSpacing:1}}>{mz}</span>
                                  {(tr.mezzi[mi]?.motivo)&&mi>0&&(
                                    <span style={{fontSize:10,color:t.textMuted}}>({tr.mezzi[mi].motivo})</span>
                                  )}
                                </span>
                              ))}
                            </div>
